...
const getAnuncioById = async (req, res, next) =>{
  ...
  try {
    ...
    if(anuncioId) {
      const anuncioById = await totalAnuncios.filter(anuncio => anuncio.id.toLowerCase().includes(anuncioId.toLowerCase()));
      anuncioById.length?
...
const getAllAnuncios = async (req, res, next) => {
  try {
    const totalAnuncios = await getAnuncios();
      totalAnuncios.length > 0 ? res.status(200).send(totalAnuncios) : res.status(404).send('No se encontraron anuncios');
  } catch (error) {
    console.error(error);
    next(error);
  }
...
module.exports = {
  getAllAnuncios, getAnuncioById, postAnuncio, putAnuncio, deleteAnuncio
}